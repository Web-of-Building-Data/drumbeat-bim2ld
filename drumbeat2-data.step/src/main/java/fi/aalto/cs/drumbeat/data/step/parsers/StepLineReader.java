...
//	private static final Pattern STATEMENT = Pattern.compile("([^;\\']|(\\'([^\\']|(\\'\\'))*\\'))+;");
	...
	public StepLineReader(InputStream input) {
	...
	public String getNextStatement() throws IOException, DrbFormatException {
		...
		for (;;) {			
			...
			if ((length = cache.length()) > 0) {
				int indexOfEndLine = cache.indexOf(StepVocabulary.SpfFormat.END_LINE);
				
				if (indexOfEndLine >= 0) {
					
					int indexOfApostrophe = cache.lastIndexOf(StepVocabulary.SpfFormat.STRING_VALUE, indexOfEndLine);
